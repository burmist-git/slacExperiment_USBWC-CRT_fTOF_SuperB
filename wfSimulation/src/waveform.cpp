...
Double_t waveform::findFirstTimePosAtDoubleLevelRiseEdge(Double_t level1, Double_t level2){
  if(level1 >= level2){
    cout<<endl<<" level1 >= level2 "<<endl
	<<"level1  "<<level1<<endl
	<<"level2  "<<level2<<endl;
      assert(0);
  }
  Int_t i = 0;
  Double_t TimeL1 = -999.0;
  Double_t TimeL2 = -999.0;
  for(i = 0;i<(_nPoints-1);i++){
    if(_wfA[i]<level1 && _wfA[i+1]>level1){
      TimeL1 = linInterpol(level1,
			   _wfA[i+1],_wfA[i],
			   _wfT[i+1],_wfT[i]);
      break;
    }
  ...
  for(i = 0;i<(_nPoints-1);i++){
    if(_wfA[i]<level2 && _wfA[i+1]>level2){
      TimeL2 = linInterpol(level2,
			   _wfA[i+1],_wfA[i],
			   _wfT[i+1],_wfT[i]);
      break;
    }
  }
  if(TimeL1>=0.0 && TimeL2>=0.0 && TimeL1 != TimeL2){
    //cout<<"TimeL1 "<<TimeL1<<endl
    //<<"TimeL2 "<<TimeL2<<endl
    //<<"level1 "<<level1<<endl
    //<<"level2 "<<level2<<endl
    //<<"_FirstAmplitudePos "<<_FirstAmplitudePos<<endl;
    return linInterpol(0,
		       level2,level1,
		       TimeL2,TimeL1);
  }
  return -999.0;
